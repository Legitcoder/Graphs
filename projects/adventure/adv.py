...
while len(graph) < len(roomGraph):
    ...
    #Use BFS and BackTrack to an unexplored room
    if dirToMove is None:
        path = find_nearest_unexplored(player.currentRoom.id, graph)
        #print(path)
        directions = []
        while path:
            current_room_id = path.pop(0)
            #print(graph[current_room_id])
            for exit in graph[current_room_id]:
                if path and graph[current_room_id][exit] == path[0]: directions.append(exit)
    ...
    #Assign the current room id to previous room after travelling
    graph[previous_room.id][dirToMove] = player.currentRoom.id

    if player.currentRoom.id not in graph: create_room(graph)

    #Assign the previous room id to current room using inverseDirection
    graph[player.currentRoom.id][inverseDirections[dirToMove]] = previous_room.id
